...
// Helper : s'assure que le plugin Owl Carousel est chargé avant d'exécuter fn
function whenOwlReady(fn){
  if (window.jQuery && jQuery.fn && jQuery.fn.owlCarousel) {
    fn();
    return;
  }
  if (window._owlLoadStarted) {
    $(document).on('owlReady', fn);
    return;
  }
  window._owlLoadStarted = true;
  $.getScript('/js/owl.carousel.min.js')
  .done(function(){
    $(document).trigger('owlReady');
    fn();
  })
  .fail(function(){
    console.warn('Échec du chargement de owl.carousel');
  });
}
    if ($('.team_slider').length>0){
      $(document).ready(function(){
        whenOwlReady(function(){
...
$(document).on('click','.copyme',function(){
    ...
    setTimeout(function(){
        // Guarded Instagram posts loader to prevent JSON.parse error if endpoint is not available
(function(){
  function safeGetInstPosts(){
    try {
      const hasEndpointFlag = typeof window.APP_ENABLE_INST !== 'undefined' ? !!window.APP_ENABLE_INST : false;
      const isProdDomain = typeof window.location !== 'undefined' && /kksmartcom\.com$/i.test(window.location.hostname);
      if (hasEndpointFlag || isProdDomain) {
        if (typeof getInstPosts === 'function') {
          getInstPosts();
        }
      }
    } catch (err) {
      console.warn('getInstPosts skipped due to error:', err);
    }
  }
  safeGetInstPosts();
})();
